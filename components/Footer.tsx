        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-[2.2fr_1fr_1fr] gap-8 lg:gap-16 mb-[52px]">
          ...
          <div>
            ...
            <div className="flex gap-[10px]">
              {SOCIAL_LINKS.map(({ label, path, href, external }) => (
                <a key={label} href={href} aria-label={label}
                  {...(external ? { target: '_blank', rel: 'noopener noreferrer' } : {})}
                  className="w-9 h-9 rounded-[6px] bg-bk-3 border border-white/[0.07] flex items-center justify-center text-lo transition-all duration-300 hover:text-gold hover:border-gold/40">
                  {label === 'Instagram' ? (
